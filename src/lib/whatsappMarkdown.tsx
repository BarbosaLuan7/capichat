  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1] !== undefined) {
      ...
      parts.push(
        <pre
          key={uniqueKey}
          className="my-1 inline-block rounded bg-background/20 px-2 py-1 font-mono text-xs"
        >
          {match[1]}
        </pre>
      );
      ...
      // Inline code `
      parts.push(
        <code key={uniqueKey} className="rounded bg-background/20 px-1 font-mono text-xs">
          {match[2]}
        </code>
      );
      ...
      // Strikethrough ~
      parts.push(
        <del key={uniqueKey} className="opacity-70">
          {match[5]}
        </del>
      );
    }
  ...
  lines.forEach((line, lineIndex) => {
    ...
    if (line.startsWith('> ')) {
      ...
      result.push(
        <span
          key={keyPrefix}
          className="border-current/50 inline-flex border-l-2 pl-2 italic opacity-80"
        >
    ...
    if (inlineParts.length === 1 && typeof inlineParts[0] === 'string') {
      ...
      result.push(<React.Fragment key={keyPrefix}>{inlineParts}</React.Fragment>);
    }
  });
