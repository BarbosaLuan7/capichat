...
export const NewConversationModal = forwardRef<HTMLDivElement, NewConversationModalProps>(function NewConversationModal(
  { open, onOpenChange, onConversationCreated },
  ref,
) {
  const { user } = useAuth();
  ...
  const getInstanceDisplay = (config: (typeof activeInstances)[0]) => {
    return (
      <div className="flex items-center gap-2">
        ...
        {config.phone_number && (
          <span className="text-muted-foreground text-xs">({formatPhoneNumber(config.phone_number)})</span>
        )}
      </div>
    );
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent ref={ref} className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Inicie um novo atendimento no WhatsApp</DialogDescription>
        </DialogHeader>
            ...
            <div className="flex gap-2">
              ...
              <div className="relative flex-1">
                <Input
                  ...
                  placeholder={countryCode === "55" ? "45 99995-7851" : "Número"}
                  className="pr-10"
                  autoFocus
                />
            ...
            {existingLead && (
              <Alert variant="destructive" className="py-2">
                <AlertDescription className="flex items-center justify-between">
                  ...
                  <Button type="button" variant="outline" size="sm" onClick={handleOpenExisting} className="ml-2 gap-1">
