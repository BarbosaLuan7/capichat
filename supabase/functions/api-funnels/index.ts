...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!id) {
      let query = supabase.from('funnel_stages').select('*').order('order', { ascending: true });
      ...
      return new Response(JSON.stringify({ dados: funnels }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (id) {
      const query = supabase.from('funnel_stages').select('*').order('order', { ascending: true });
      ...
      if (!stages || stages.length === 0) {
        return new Response(JSON.stringify({ success: false, error: 'Funnel not found' }), {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
