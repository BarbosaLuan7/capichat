  ...
  try {
    ...
    if (req.method === 'GET') {
      if (id) {
        ...
        if (error) {
          return new Response(JSON.stringify({ sucesso: false, erro: 'Campo não encontrado' }), {
            status: 404,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }

        return new Response(JSON.stringify(formatCampo(data)), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (error) {
        return new Response(JSON.stringify({ sucesso: false, erro: 'Erro ao listar campos' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      return new Response(JSON.stringify({ dados: (data || []).map(formatCampo) }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (req.method === 'POST') {
      ...
      if (error) {
        ...
        return new Response(JSON.stringify({ sucesso: false, erro: 'Erro ao criar campo' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      return new Response(JSON.stringify({ sucesso: true, campo: formatCampo(data) }), {
        status: 201,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (req.method === 'PUT') {
      ...
      if (error) {
        return new Response(JSON.stringify({ sucesso: false, erro: 'Erro ao atualizar campo' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (req.method === 'DELETE') {
      ...
      if (error) {
        return new Response(JSON.stringify({ sucesso: false, erro: 'Erro ao remover campo' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
