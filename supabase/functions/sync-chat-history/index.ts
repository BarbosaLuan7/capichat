...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      '[sync-chat-history] Iniciando sync para conversa:',
      conversation_id,
      'limit:',
      limit
    );
    ...
    if (!lead) {
      return new Response(JSON.stringify({ error: 'Lead not found for conversation' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!wahaConfig) {
      ...
      if (wahaConfig) {
        console.log(
          '[sync-chat-history] Usando fallback - primeira instância ativa:',
          wahaConfig.instance_name
        );
      }
    }
    ...
    const phoneWithCountry = lead.phone.startsWith(countryCode)
      ? lead.phone
      : `${countryCode}${lead.phone}`;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(
          '[sync-chat-history] Erro ao buscar mensagens do WAHA:',
          response.status,
          errorText
        );
    ...
    for (const msg of wahaMessages) {
      try {
        ...
        errors.push(
          `Msg ${msg.id}: ${msgError instanceof Error ? msgError.message : 'Unknown error'}`
        );
      }
    }
