    ...
    try {
      ...
      switch (action.type) {
        ...
        case 'create_task': {
          const leadId = (data.lead as Record<string, unknown>)?.id as string;
          const assignedTo =
            (action.params.assigned_to as string) ||
            ((data.lead as Record<string, unknown>)?.assigned_to as string);
        ...
        case 'notify_user': {
          const userId =
            (action.params.user_id as string) ||
            ((data.lead as Record<string, unknown>)?.assigned_to as string);
          const title = action.params.title as string;
...
async function fetchProfilePicture(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(
        `[avatar-retry] ❌ API retornou ${response.status}: ${errorText.substring(0, 100)}`
      );
...
async function processAvatarRetry(
  ...
  console.log(
    `[avatar-retry] 📷 Processando tentativa ${attempt} para lead ${lead_id}, contact_id=${contact_id}, is_lid=${is_lid}`
  );
  ...
  const result = await fetchProfilePicture(
    baseUrl,
    wahaConfig.api_key,
    wahaConfig.instance_name || session,
    contact_id,
    is_lid
  );
  ...
  if (attempt < maxAttempts) {
    console.log(
      `[avatar-retry] 📷 Agendando tentativa ${attempt + 1}/${maxAttempts} para lead ${lead_id}`
    );
    ...
    console.log(
      `[avatar-retry] ⚠️ Máximo de tentativas atingido para lead ${lead_id}. Motivo: ${result.reason}`
    );
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!queueItems || queueItems.length === 0) {
      ...
      return new Response(JSON.stringify({ message: 'No items to process', processed: 0 }), {
